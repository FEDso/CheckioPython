...
If you have two or more letters with the same frequency, then return the letter which comes first in the latin alphabet. For 
example -- "one" contains "o", "n", "e" only once for each, thus we choose "e".
...
def checkio(string: str) -> str:
    string = Counter(string.lower())
    for (k, v) in sorted(string.items(), key=lambda x: x[1], reverse=True):
        if ord('a') <= ord(k) <= ord('z'):
            count = v 
            break
    letters = [k for (k, v) in string.items() if v == count and ord('a') <= ord(k) <= ord('z')]
