"""
You are the owner of a cafe where 3 chefs work: a JapaneseCook, RussianCook and ItalianCook. Each of them can prepare the 
national food and beverage:
- JapaneseCook: Sushi and Tea;
- RussianCook: Dumplings and Compote;
- ItalianCook: Pizza and Juice.
Your task is to create 3 different subclasses (one for each chef) which are the children of an AbstractCook and have these 
methods:
...
- total(), which returns a string like: 'Foods: 150, Drinks: 50, Total: 200', and for the each chef instead of the Foods and 
Drinks will be the national food and drink that he’s used.
...
How it is used: Work with classes and object-oriented programming is considered to be on a much higher skill level which you 
should reach in order to put Python to full use.
...
class AbstractCook:
    def add_food(self, amount, price):
        self.products[0][1] += amount*price
        
    def add_drink(self, amount, price):
        self.products[1][1] += amount*price
        
    def total(self):
        food = self.products[0][0] + ': ' + str(self.products[0][1])
        drink = self.products[1][0] + ': ' + str(self.products[1][1])
        total = 'Total: ' + str(self.products[0][1] + self.products[1][1])
        return food + ', ' + drink + ', ' + total

class JapaneseCook(AbstractCook):
    def __init__(self):
        self.products = (['Sushi', 0], ['Tea', 0])
...
class RussianCook(AbstractCook):
    def __init__(self):
        self.products = (['Dumplings', 0], ['Compote', 0])
